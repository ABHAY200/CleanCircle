import { useForm } from 'react-hook-form';
import { Link, useNavigate } from 'react-router-dom';
import { motion } from 'framer-motion';
import { APP } from '@/constants/app';
import { TEXT } from '@/constants/text';
import { ROUTES } from '@/constants/routes';
import { ROLES } from '@/constants/roles';
import { Input } from '@/components/forms/FormField';
import { Button } from '@/components/buttons/Button';
import { useAppDispatch, useAppSelector } from '@/redux/hooks';
import { loginUser, selectAuthLoading, selectAuthError, clearError } from '@/redux/auth/authSlice';
import { addToast } from '@/redux/ui/uiSlice';
import { Logo } from '@/components/common/Logo';
import { AppImage } from '@/components/common/AppImage';
import type { LoginFormData } from '@/types';
import { useEffect } from 'react';

export function Login() {
  const dispatch = useAppDispatch();
  const navigate = useNavigate();
  const isLoading = useAppSelector(selectAuthLoading);
  const error = useAppSelector(selectAuthError);
  const { register, handleSubmit, formState: { errors } } = useForm<LoginFormData>();

  useEffect(() => {
    dispatch(clearError());
  }, [dispatch]);

  const onSubmit = async (data: LoginFormData) => {
    const result = await dispatch(loginUser(data));
    if (loginUser.fulfilled.match(result)) {
      dispatch(addToast({ type: 'success', message: TEXT.AUTH.LOGIN_SUCCESS }));
      const role = result.payload.role;
      if (role === ROLES.CLEANER) navigate(ROUTES.CLEANER.DASHBOARD);
      else if (role === ROLES.ADMIN) navigate(ROUTES.ADMIN.DASHBOARD);
      else navigate(ROUTES.CUSTOMER.DASHBOARD);
    }
  };

  return (
    <div className="min-h-[calc(100vh-4rem)] flex">
      <div className="hidden lg:flex lg:w-1/2 relative bg-gray-200">
        <AppImage
          src="https://images.unsplash.com/photo-1581578731548-c64695cc6952?w=1200&q=80"
          alt="Cleaning"
          showIconOnError
          className="w-full h-full object-cover"
        />
        <div className="absolute inset-0 bg-primary/60 flex items-center justify-center p-12">
          <div className="text-white text-center">
            <Logo size="xl" showText={false} linkToHome={false} className="justify-center mb-6" imageClassName="drop-shadow-lg" />
            <h2 className="text-3xl font-bold">{APP.NAME}</h2>
            <p className="text-blue-100 mt-2 text-lg">{APP.TAGLINE}</p>
          </div>
        </div>
      </div>

      <div className="flex-1 flex items-center justify-center p-6 sm:p-12">
        <motion.div
          initial={{ opacity: 0, x: 20 }}
          animate={{ opacity: 1, x: 0 }}
          className="w-full max-w-md"
        >
          <h1 className="text-2xl font-bold text-text">{TEXT.LOGIN.TITLE}</h1>
          <p className="text-text-muted mt-2">{TEXT.LOGIN.SUBTITLE}</p>

          {error && (
            <div className="mt-4 rounded-xl bg-red-50 border border-red-200 p-3 text-sm text-red-700">
              {error}
            </div>
          )}

          <form onSubmit={handleSubmit(onSubmit)} className="mt-8 space-y-5">
            <Input
              label={TEXT.LOGIN.EMAIL}
              type="email"
              placeholder={TEXT.LOGIN.EMAIL_PLACEHOLDER}
              {...register('email', { required: 'Email is required' })}
              error={errors.email?.message}
            />
            <Input
              label={TEXT.LOGIN.PASSWORD}
              type="password"
              placeholder={TEXT.LOGIN.PASSWORD_PLACEHOLDER}
              {...register('password', { required: 'Password is required' })}
              error={errors.password?.message}
            />
            <div className="flex items-center justify-between">
              <label className="flex items-center gap-2 text-sm">
                <input type="checkbox" {...register('remember')} className="rounded accent-primary" />
                {TEXT.LOGIN.REMEMBER}
              </label>
              <a href="#" className="text-sm text-primary hover:underline">{TEXT.LOGIN.FORGOT}</a>
            </div>
            <Button type="submit" fullWidth size="lg" isLoading={isLoading}>
              {TEXT.LOGIN.SUBMIT}
            </Button>
          </form>

          <div className="relative my-6">
            <div className="absolute inset-0 flex items-center"><div className="w-full border-t border-border" /></div>
            <div className="relative flex justify-center text-sm"><span className="bg-background px-4 text-text-muted">{TEXT.COMMON.OR}</span></div>
          </div>

          <Button variant="outline" fullWidth size="lg">
            <svg className="h-5 w-5" viewBox="0 0 24 24"><path fill="currentColor" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 01-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z"/><path fill="currentColor" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/><path fill="currentColor" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/><path fill="currentColor" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/></svg>
            {TEXT.LOGIN.GOOGLE}
          </Button>

          <p className="text-center text-sm text-text-muted mt-4 bg-amber-50 border border-amber-200 rounded-xl p-3">
            {TEXT.LOGIN.DEMO_HINT}
          </p>

          <p className="text-center text-sm text-text-muted mt-6">
            {TEXT.LOGIN.NO_ACCOUNT}{' '}
            <Link to={ROUTES.REGISTER} className="text-primary font-medium hover:underline">
              {TEXT.LOGIN.REGISTER}
            </Link>
          </p>
        </motion.div>
      </div>
    </div>
  );
}
